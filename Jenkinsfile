pipeline {
	...
	stages {
		...
		stage('Deploy artifact to Cloud Foundry') {
			...
			steps {
				echo 'gradle cf-push -Pcf.host=${params.APP_NAME}-${params.CF_SPACE} -Pcf.ccHost=${params.CF_API} -Pcf.domain=${params.CF_DOMAIN} -Pcf.ccUser=$CF_USERNAME -Pcf.ccPassword=**** -Pcf.org=${params.ORGANIZATION} -Pcf.space=${params.CF_SPACE}'
